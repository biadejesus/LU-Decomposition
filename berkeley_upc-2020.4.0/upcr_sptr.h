...
/* Indirections left-over from TV support */
...
/*
 * At least GASNet's udp-conduit includes sockets.h and at least Solaris 11.2
 * #defines s_addr there, causing big problems when we use it a field name.
 */
#ifdef s_addr
  #undef s_addr
#endif
 ...
 * are responsible for ensuring these get defined somewhere.
 */
...
GASNETT_INLINE(upcri_shared_to_remote)
void *
upcri_shared_to_remote(upcr_shared_ptr_t sptr)
{
    (void) upcri_checkvalid_shared(sptr);
...
GASNETT_INLINE(upcri_shared_to_remote_off)
void *
upcri_shared_to_remote_off(upcr_shared_ptr_t sptr, ptrdiff_t offset)
{
    (void) upcri_checkvalid_shared(sptr);
...
GASNETT_INLINE(upcri_pshared_to_remote)
void *
upcri_pshared_to_remote(upcr_pshared_ptr_t sptr)
{
    (void) upcri_checkvalid_pshared(sptr);
    #if UPCRI_SYMMETRIC_PSHARED
      return (void*) (sptr); /* free */
...
GASNETT_INLINE(upcri_pshared_to_remote_off)
void *
upcri_pshared_to_remote_off(upcr_pshared_ptr_t sptr, ptrdiff_t offset)
{
    (void) upcri_checkvalid_pshared(sptr);
    #if UPCRI_SYMMETRIC_PSHARED
...
GASNETT_INLINE(upcri_shared_to_remote_withthread)
void *
upcri_shared_to_remote_withthread(upcr_shared_ptr_t sptr, upcr_thread_t threadno)
{
    (void) upcri_checkvalid_shared(sptr);
...
GASNETT_INLINE(upcri_pshared_to_remote_withthread)
void *
upcri_pshared_to_remote_withthread(upcr_pshared_ptr_t sptr, upcr_thread_t threadno)
{
    (void) upcri_checkvalid_pshared(sptr);
    upcri_assert(threadno < upcr_threads());
    return (void *) (upcr_addrfield_pshared(sptr)
...
GASNETT_INLINE(upcr_local_to_shared_withphase)
upcr_shared_ptr_t 
upcr_local_to_shared_withphase(void *lptr, upcr_phase_t phase, 
...
GASNETT_INLINE(upcr_local_to_shared_ref_withphase)
void 
upcr_local_to_shared_ref_withphase(void *lptr, upcr_phase_t phase, 
		    upcr_thread_t threadid, upcr_shared_ptr_t *ref) 
{
    if (lptr == NULL) 
...
GASNETT_INLINE(upcr_affinitysize)
size_t 
upcr_affinitysize(size_t totalsize, size_t nbytes, upcr_thread_t threadid) 
{
...
GASNETT_INLINE(upcr_add_pshared1)
upcr_pshared_ptr_t
upcr_add_pshared1(upcr_pshared_ptr_t sptr, size_t elemsz, ptrdiff_t inc)
{
    #if PLATFORM_COMPILER_GCC && PLATFORM_COMPILER_VERSION_EQ(3,2,3) && PLATFORM_ARCH_X86_64
      /* bug1486: workaround a gcc optimizer crash tickled by this code on XT-3 */
      long numthreads = upcri_threads;
...
GASNETT_INLINE(upcr_sub_shared)
ptrdiff_t 
upcr_sub_shared(upcr_shared_ptr_t sptr1, upcr_shared_ptr_t sptr2, 
