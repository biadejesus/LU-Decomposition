...
/* allows attaching debugger as early as possible after gex_Init_Client */
static void 
...
" - To debug additional nodes, attach to them before unfreezing node %d.\n",
	      (int)upcri_mynode, (int)pid, gasnett_gethostname(), (int)upcri_mynode);
...
" - Note: if you wish to set a breakpoint at 'main', use '%s' instead.\n", 
	...
	printf("Node %d (pid %d on %s) waiting for node %d\n",
	       (int)upcri_mynode, (int)pid, gasnett_gethostname(), freezenode);
	fflush(stdout);
	sleep(1);
    }
...
"-----------------------------------------------------------------------\n"
  ...
  UPCRI_SINGLE_BARRIER_NOTHR();
  { int threadwidth = ((int)log10(MAX(1,upcri_1stthread(upcri_nodes-1))))+1;
    int hthreadwidth = ((int)log10(MAX(1,upcr_threads()-1)))+1;
    int processwidth = ((int)log10(upcri_nodes))+1;
    ...
    char *sninfo;
    {
      ...
      snprintf(sninfo, len, fmt,
               snwidth, (int)upcri_nodeinfo[upcri_mynode].supernode,
               snwidth, max_sn+1);
    }
 #else
    char sninfo[] = "";
    ...
    if (upcri_mypthreads() == 1) {
      printf("UPCR: UPC thread %*i of %i on %s (%sprocess %*d of %*d, pid=%lu)\n",
         threadwidth, (int)upcri_1stthread(upcri_mynode), 
         (int)upcr_threads(), 
         gasnett_gethostname(), sninfo,
	 processwidth, (int)upcri_mynode, 
         processwidth, (int)upcri_nodes, 
         (unsigned long)mypid);
    } else {
      printf("UPCR: UPC threads %*i..%*i of %i on %s (%sprocess %*d of %*d, pid=%lu)\n",
         threadwidth, (int)upcri_1stthread(upcri_mynode), 
         hthreadwidth, (int)(upcri_1stthread(upcri_mynode) + upcri_mypthreads() - 1), 
	 (int)upcr_threads(), 
         gasnett_gethostname(), sninfo,
	 processwidth, (int)upcri_mynode, 
         processwidth, (int)upcri_nodes, 
		...
		upcr_thread_t default_pthreads_per_proc)
{
	...
	upcri_thread2node = UPCRI_XMALLOC_EARLY(gex_Rank_t, upcri_threads);
	upcri_thread2pthread = UPCRI_XMALLOC_EARLY(upcri_pthread_t, upcri_threads);
	upcri_node2pthreads = UPCRI_XMALLOC_EARLY(upcri_pthread_t, upcri_nodes);
	upcri_node1stthread = UPCRI_XMALLOC_EARLY(upcri_pthread_t, upcri_nodes+1);
	...
	upcri_threads = upcri_nodes * pthreads_per;
	upcri_thread2node = UPCRI_XMALLOC_EARLY(gex_Rank_t, upcri_threads);
	upcri_thread2pthread = UPCRI_XMALLOC_EARLY(upcri_pthread_t, upcri_threads);
	...
	upcri_node2pthreads = UPCRI_XMALLOC_EARLY(upcri_pthread_t, upcri_nodes);
	upcri_node1stthread = UPCRI_XMALLOC_EARLY(upcri_pthread_t, upcri_nodes+1);
	for (i = 0; i < upcri_nodes; i++) {
	    upcri_node2pthreads[i] = pthreads_per;
	    upcri_node1stthread[i] = i * pthreads_per;
	}
	upcri_node1stthread[upcri_nodes] = upcri_threads;
    }
    upcri_mypthread_cnt = upcri_node2pthreads[upcri_mynode];
#else
    upcri_threads = upcri_nodes;
#endif
    ...
    if ((uint64_t)upcri_nodes > (uint64_t)UPCR_MAXNODES)
       upcri_err("too many runtime nodes! UPCR_MAXNODES=%llu, node count=%llu",
                 (unsigned long long)UPCR_MAXNODES, (unsigned long long)upcri_nodes);
		       ...
		       const char * main_name)
{
    ...
    if ( (err = gex_Client_Init(&upcri_client, &upcri_ep, &upcri_tm, "UPCR", pargc, pargv, GEX_FLAG_USES_GASNET1)))
       upcri_gaserr(err, "gex_Client_Init failed!");
#ifdef GASNET_STATS
    ...
    upcri_mynode = gex_TM_QueryRank(upcri_tm);
    upcri_nodes = gex_TM_QuerySize(upcri_tm);
...
static int upcri_use_gasnet_exit = 1; // use gasnet_exit for collective exits?
...
static void upcri_atexit_handler(void)
{
    ...
    if (!upcri_gasnet_exit_called && upcri_use_gasnet_exit) {
...
void upcri_remoteExitSignalHandler(int sig) {
  ...
  struct upcri_seginfo *upcri_seginfos;
    ...
    void upcri_SRQ_seginfo(gex_Token_t token, void *info_addr, void *seg_addr, void *seg_size) {
      gex_Token_Info_t info;
      gex_Token_Info(token, &info, GEX_TI_SRCRANK);
      gex_Rank_t srcnode = info.gex_srcrank;

      upcri_assert(srcnode != upcri_mynode);
...
"upcr_startup_attach: default_shared_offset %lu not a multiple of UPCR_PAGESIZE",
    ...
    /* validate shared heap size parameter */
    perthread_req = (flags & UPCR_ATTACH_ENV_OVERRIDE)
                    ? gasnett_getenv_int_withdefault("UPC_SHARED_HEAP_SIZE", default_shared_size, MEGABYTE)
                    : default_shared_size;
    if (perthread_req < 1024)
        upcri_err("illegal size for UPC_SHARED_HEAP_SIZE: %s",
                  gasnett_format_number(perthread_req, buf, sizeof(buf), 1));
#if UPCRI_PACKED_SPTR && (UPCRI_ADDR_BITS < 8*SIZEOF_VOID_P)
    if (perthread_req > ((uintptr_t)1 << UPCRI_ADDR_BITS))
        upcri_err("out-of-range size for UPC_SHARED_HEAP_SIZE: %s (sptr representation)",
                  gasnett_format_number(perthread_req, buf, sizeof(buf), 1));
#else
    if (((uintptr_t)perthread_req) != perthread_req) /* check for size > 32 bits on ILP32 */
        upcri_err("out-of-range size for UPC_SHARED_HEAP_SIZE: %s (32-bit ABI)",
                  gasnett_format_number(perthread_req, buf, sizeof(buf), 1));
#endif

    /* validate shared heap offset parameter */
    heapoffset = (flags & UPCR_ATTACH_ENV_OVERRIDE)
                 ? gasnett_getenv_int_withdefault("UPC_SHARED_HEAP_OFFSET", default_shared_offset, MEGABYTE)
                 : default_shared_offset;
    /* 0 is OK */
    if (((uintptr_t)heapoffset) != heapoffset) /* check for size > 32 bits on ILP32 */
        upcri_err("out-of-range size for UPC_SHARED_HEAP_OFFSET: %s",
                  gasnett_format_number(heapoffset, buf, sizeof(buf), 1));
	...
	upcri_assert(UPCRI_IS_POWER_OF_TWO(node_req));
        perthread_req = node_req / upcri_mypthreads();
	upcri_assert(UPCRI_IS_POWER_OF_TWO(perthread_req));
    }
    #endif

    /* Initial local heap size */
    upcri_localheap_initsz = upcri_roundup_pagesz(
                                 gasnett_getenv_int_withdefault(
                                     "UPC_SHARED_LOCALHEAP_INITSZ",
                                     upcri_localheap_initsz, MEGABYTE));

    /* Page-align initial global heap size */
    upcri_globalheap_initsz = upcri_roundup_pagesz(upcri_globalheap_initsz);

    upcri_memcheck_all(); /* verify heap sanity */
#ifndef GASNET_SEGMENT_EVERYTHING
    if ((err = gex_Segment_Attach(&upcri_segment, upcri_tm, node_req)))
	upcri_gaserr(err, "gex_Segment_Attach failed!");
#endif

    // TODO: per-subsystem registration is now possible
    if ((err = gex_EP_RegisterHandlers(    upcri_ep,
                                           upcri_get_handlertable(),
                                           upcri_get_handlertable_count())))
        upcri_gaserr(err, "AM handler registration failed!");
    UPCRI_SINGLE_BARRIER_NOTHR(); /* TODO: omit or move later (but before first AM) */

    seginfos = UPCRI_XMALLOC(gasnet_seginfo_t, upcri_nodes);
    upcri_nodeinfo = UPCRI_XMALLOC(gasnet_nodeinfo_t, upcri_nodes);
    ...
    upcri_use_gasnet_exit = gasnett_getenv_yesno_withdefault("GASNET_CATCH_EXIT", 1);
    ...
    if (gasnet_getNodeInfo(upcri_nodeinfo, upcri_nodes) != GASNET_OK)
    ...
    if (gasnet_getSegmentInfo(seginfos, upcri_nodes) != GASNET_OK)
     ...
     */
    {
      ...
      upcri_seginfos = UPCRI_XMALLOC(struct upcri_seginfo, upcri_nodes);
      myseginfo = upcri_seginfos + upcri_mynode;
      myseginfo->addr = heap;
       ...
       */
      if (!upcri_mynode)
        upcri_seginfo_addrs = UPCRI_XMALLOC(struct upcri_seginfo *, upcri_nodes);
      UPCRI_SINGLE_BARRIER_NOTHR();
      if (upcri_mynode) {
        /* Send my info to 0 */
        gex_AM_RequestShort(upcri_tm, 0,
                       UPCRI_HANDLER_ID(upcri_SRQ_seginfo), 0,
                       UPCRI_SEND_PTR(upcri_seginfos),
                       UPCRI_SEND_PTR(myseginfo->addr),
                       UPCRI_SEND_PTR((void*)myseginfo->size));
      } else {
        /* wait for all arrivals */
        GASNET_BLOCKUNTIL((int)gasnett_atomic_read(&upcri_seginfos_rcvd, 0) == (upcri_nodes - 1));
        /* Bcast, excluding self */
        for (n = 1; n < upcri_nodes; ++n) { /* Bcast, excluding self */
          gex_RMA_PutNBI(upcri_tm, n, upcri_seginfo_addrs[(int)n], upcri_seginfos, upcri_nodes*sizeof(struct upcri_seginfo), GEX_EVENT_DEFER, 0);
        }
    ...
    { gex_Rank_t myhost = upcri_nodeinfo[(int)upcri_mynode].host;
      gex_Rank_t lead_node = 0;
      ...
      if (politedefault && (upcri_mynode == lead_node)) {
  #if UPCRI_UPC_PTHREADS
      ...
      } else if (upcri_polite_wait && upcri_mynode == 0) {
...
"or set the UPC_SHARED_HEAP_SIZE or UPC_SHARED_HEAP_OFFSET environment variables.\n",
			...
			(unsigned long)(seginfos[n].size/MEGABYTE));
                ...
                // other nodes will see non-collective exit no later than next barrier
	    }
	}
    ...
    #if !UPCRI_UPC_PTHREADS
    upcri_myregion_single = (uintptr_t)seginfos[upcri_mynode].addr;
      #if UPCR_USING_LINKADDRS
	...
	t = upcri_1stthread(upcri_mynode);
	...
	for ( ; t <= maxthread; ++t) {
      ...
      #if UPCRI_USING_PSHM
        for(n = 0; n < upcri_nodes; n++){
            if (n == upcri_mynode) continue;
            if (upcri_nodeinfo[upcri_mynode].supernode == upcri_nodeinfo[n].supernode) {
	...
	for (n = 0; n < upcri_nodes; n++) {
        ...
        upcri_thread2local[upcri_mynode] = 0 UPCRI_PLUS_MY_OFFSET;
    ...
    /* Setup upcri_shared_thread(s) to enumerate directly addressable threads */
      ...
      { /* Most general case: any thread might be addressable */
        const gex_Rank_t supernode = upcri_nodeinfo[(int)upcri_mynode].supernode;
        int i = 0;
...
static void *upcri_perthread_spawn(void *args)
{
    ...
    _upcr_pthreadinfo->mygasnet_tinfo = GASNET_GET_THREADINFO();
  ...
  #define UPCRI_SUPPRESS_SIGFPE 0
    ...
    /* Validate heap size */
    uintptr_t heap_initsz = upcri_localheap_initsz + upcri_globalheap_initsz;
    if (upcri_perthread_heapsize <= heap_initsz) {
      if(!upcr_mythread()) {
        upcri_err("Per-thread shared heap size of %lu MB is too small by at least %lu MB.\n",
                  (unsigned long)(upcri_perthread_segsize/MEGABYTE),
                  (unsigned long)(MAX(1,(heap_initsz - upcri_perthread_heapsize)/MEGABYTE)));
      }
      // other UPC threads will see non-collective exit no later than next barrier
    }
    ...
    { const char *trace_id;
      ...
      GASNETT_TRACE_PRINTF_FORCE(
		...
		(int)upcri_mynode, (int)upcri_nodes,
		hostname, trace_id);
    }
    #endif
 ...
 * Not a good idea if you want performance, but was needed for Totalview.
 * Retained as harmless, for now. */

static int upcri_tv_progress_verbose = 0; /* for debugging TV support */
static pthread_t upcri_tv_progress_thread;
static int upcri_tv_progress_exit = 0;

void *upcri_run_progress_thread(void *ignoreme)
{
    while (! upcri_tv_progress_exit) {
			...
			struct upcr_startup_spawnfuncs *spawnfuncs)
{
    ...
    if (UPCRL_progress_thread) {
        ...
        upcri_tv_progress_thread = upcri_create_thread(&upcri_run_progress_thread, NULL);
    }
#endif
	...
	arg->mypthread = p;
	arg->mythread = upcri_1stthread(upcri_mynode) + p;
	...
	*arg->spawnfuncs = *spawnfuncs;
    ...
    mainarg.mypthread = 0;
#endif
    mainarg.mythread = upcri_1stthread(upcri_mynode);
    mainarg.argc = *pargc;
    mainarg.argv = *pargv;
...
upcr_global_exit(int exitcode) 
{
...
#if UPCRI_SUPPORT_PTHREADS
    if (UPCRL_progress_thread) {
      upcri_tv_progress_exit = 1;
      pthread_join(upcri_tv_progress_thread, NULL);
    }
#endif
    ...
    if (UPCRL_profile_finalize != NULL) {
        ...
        if (UPCRL_profile_finalize != NULL) {
      #endif
          if (upcri_nodes > 1 && !upcr_getenv("UPC_QUIET"))
...
upcri_do_exit(int exitval)
{
    ...
    #undef UPCRI_PEVT_ARGS
      ...
      upcri_pevt_isinit = 0;
      if (upcri_nodes > 1) { /* wait for all nodes to finish finalizing GASP */
         (void) upcri_finalbarrier(0);
      }
    #endif
	...
	if (upcri_mypthread() == 0) { 
        ...
        gasnett_flush_streams();
        upcri_pthread_barrier();
        if (upcri_nodes > 1) { /* wait for all nodes to write profile */
          (void) upcri_finalbarrier(0);
        }
    }
    ...
    if (upcri_finalbarrier((5 + upcri_nodes / 64) * 1E6)) {
    ...
    if (!upcri_use_gasnet_exit) exit(exitval);
		      ...
		      size_t count UPCRI_PT_ARG)
{
	...
	for (i = 0; i < count; i++) {
		...
		continue;
            ...
            for (int j = 0; j < i; j++) { // bug 3776: detect and ignore duplicates in infos
              // Current algorithm incurs initialization-time overhead quadratic in the
              // number of static shared variables in the input (ie declared within one
              // translation unit). If this ever becomes a bottleneck, the prior linear
              // behavior could be restored by introducing four new "dummy" PTS values
              // to store in the proxy pointers while the algorithm runs (to serve as
              // an in-progress flag for {init,null} x {shared,pshared}), but this
              // entails significant additional complexity that does not seem justified
              // at present.
              if (pptr == infos[j].sptr_addr) {
                upcr_setnull_shared(&sptrbuf[i]);
                goto next;
              }
            }
	...
	for (i = 0; i < count; i++) {
	    ...
	    if (upcr_is_init_shared(*pptr) || upcr_isnull_shared(*pptr)) {
                upcri_assert(!upcr_isnull_shared(sptrbuf[i]));
		*pptr = sptrbuf[i];
		       ...
		       size_t count UPCRI_PT_ARG)
{
	...
	for (i = 0; i < count; i++) {
		...
		continue;
            ...
            for (int j = 0; j < i; j++) { // bug 3776: detect and ignore duplicates in infos
              if (pptr == infos[j].psptr_addr) {
                upcr_setnull_pshared(&sptrbuf[i]);
                goto next;
              }
            }
	...
	for (i = 0; i < count; i++) {
	    upcr_pshared_ptr_t *pptr = infos[i].psptr_addr;
	    if (upcr_is_init_pshared(*pptr) || upcr_isnull_pshared(*pptr)) {
                upcri_assert(!upcr_isnull_pshared(sptrbuf[i]));
		*pptr = sptrbuf[i];
