...
#include <gasnet_vis.h>
  ...
  static void _upcri_coll_strided_get(void *dst, upcr_thread_t src_th, void *src,
    ...
    gex_Rank_t src_node = upcri_thread_to_node(src_th);
    UPCRI_PASS_GAS();

    if (src_node == upcri_mynode) {
	...
	if (skip) {
	    if (skip < len) {
		size_t count = MIN(nelems, len-skip);
		Not-updated-for-ex: gasnet get_nbi_bulk((void *)(dst_addr + skip * elem_size),
	...
	/* Take care of trailing final elements (if any) */
	if (nelems) {
	    Not-updated-for-ex: gasnet get_nbi_bulk((void *)dst_addr, src_node, (void *)src_addr, nelems * elem_size);
	}
...
UPCRI_COLL_REDUCE_NC_PROTO(UPCRI_COLL_RED_TYPE,UPCRI_COLL_RED_SUFF)
{
	...
	upcr_shared_ptr_t gath_sptr = UPCR_NULL_SHARED; /* init to suppress a warning */
...
UPCRI_COLL_PREFIX_REDUCE_PROTO(UPCRI_COLL_RED_TYPE,UPCRI_COLL_RED_SUFF)
{
	...
	if (my_count != 0) {
		...
		if (upcri_thread_is_addressable(src_th)) {
		    ...
		    gex_RMA_GetBlocking(upcri_tm, my_dst, upcri_thread_to_node(src_th), my_src, nbytes, 0);
		}
	    } else {
#if 1
