...
my (@all_networks) = qw/mpi udp smp ibv aries ofi ucx/;

# Networks we do not advertise support for
my (@hidden_networks) = qw/ofi ucx/;
...
my %conf = (
    ...
    uses_threads_support    => 'nodefault',
...
my $struct_promote;
# alignof __uint128_t if we need it
my $alignof_uint128_t;
...
# set to 'seq', 'par', or 'thr', depending on use of pthreads
my $parseq;
...
EOF
    unless ($h2mhelp) {
        print <<EOF;
For detailed documentation, please see man upcc(1) or https://upc.lbl.gov/docs/
EOF
    }
    print <<EOF; 
...
Advanced options:
   ...
   -[no]fast-symptr   Use fast symmetric pointers for power-of-two static 
		      ...
		      If available, on by default if -T passed a power-of-two value.
...
sub print_compiler_version_item 
{
    ...
    my $tmpexec = "$TMPDIR/upcc-compilerver-$luser-$$";
    open (TMPDUMMY, ">$tmpfile"); 
    if (-f "$upcr_include_src/gasnet_portable_platform.h") {
      print TMPDUMMY "#define PLATFORM_SHOW 1\n#include <$upcr_include_src/gasnet_portable_platform.h>\n";
    } elsif (-f "$upcr_include_src/gasnet/other/portable_inttypes.h") {
      print TMPDUMMY "#define PLATFORM_SHOW 1\n#include <$upcr_include_src/gasnet/other/gasnet_portable_platform.h>\n";
    } else {
    ...
    # the generic algorithm is carefully chosen to work across all known compiler version outputs
    # picks up optional trailing line after version identifier for several compilers
    # however, we also prune and/or permute the search for faster/better with the most common compilers
    # -qversion must come first for xlc
    # -V should precede -v and --version or Intel C
    # --version should precede -v for gcc (and work-alikes)
    # TODO: move the option selection logic to configure time?
    my $with_tmpfile = "$tmpfile -o $tmpexec";
    my @vers_opts = ();
    if ($family eq 'CRAY') {
      @vers_opts = qw/-V/;
    } elsif ($family eq 'XLC') {
      @vers_opts = qw/-qversion --version/;
    } elsif ($family eq 'INTEL') {
      @vers_opts = ("-V", "-V $with_tmpfile", "-v", "-v $with_tmpfile", "--version");
    } elsif (grep($family eq $_, qw(PGI SUN))) {
      @vers_opts = qw/--version -V/;
    } elsif (grep($family eq $_, qw(GNU CLANG OPEN64 PATHSCALE))) {
      @vers_opts = qw/--version -v/;
    } else {
      @vers_opts = ("-qversion", "-V", "-V $with_tmpfile", "--version", "-v", "-v $with_tmpfile");
    }
    my $val = undef;
    my $maxlines = 5;
    foreach my $opt (@vers_opts) {
    ...
    $val =~ s/^.*in function.*(_start|main).*$//im; # icc
...
sub initialize 
{
	...
	'ansi-alias'        => \$opt_ansi_alias,		       
        ...
        'heap-offset=s'     => sub { print "upcc: warning: obsolete -heap-offset option ignored.\n"; },
        'require-size!'     => \$opt_require_size,
        'shared-heap=s'     => \$opt_shared_heap,
        'shared-heap-max=s' => sub { print "upcc: warning: obsolete -shared-heap-max option ignored.\n"; },
        ...
        'tv'                => sub { # catch attempts of older clients use -tv with this upcc acting as http server
                               die "Totalview is no longer supported in the selected version. Please use Berkeley UPC 2.28.0 or older.\n"; },
...
#        'W=s'              # handled by do_ARGVoodoo
	...
	if ($conf{pthreads_support} !~ /par/i) {
        ...
        } elsif ($conf{uses_threads_support} !~ /yes/i) {
            die "-uses-threads support is not present in this configuration of Berkeley UPC\n";
        }
    }
	...
	close (SIZES);
        ...
        # Optional
        $alignof_uint128_t = $$sizes_ref{"alignof_uint128_t"} || 0;
    ...
    $extra_cppflags   .= " -D__UPC_COLLECTIVE__=1" unless ($opt_pthreads); # Not supported in GEX
    $extra_cppflags   .= " -D__UPC_ATOMIC__=1";
...
sub do_preprocess
{
    ...
    foreach my $basename (@toPreprocess) {
	    ...
	    print DST "#pragma upc end_upc_only\n";
            ...
            print DST "#include <gasnet_portable_platform.h>\n"; # support logic in upcr_geninclude/*.h
            if ($alignof_uint128_t) {
              print DST "#ifdef UPCRI_NEED_UINT128_T\n";
              print DST "  typedef struct { char c[16]; } bupc__int128_t __attribute__((__aligned__($alignof_uint128_t)));\n";
              print DST "  #define __int128_t bupc__int128_t\n";
              print DST "  typedef struct { char c[16]; } bupc__uint128_t __attribute__((__aligned__($alignof_uint128_t)));\n";
              print DST "  #define __uint128_t bupc__uint128_t\n";
              print DST "#endif\n";
            }
...
EOF
	    ...
	    my $killsrcpos = 0;
	  	...
	  	} elsif (/^upcr_startup_(?:p)?shalloc_t (?:_bupc_)?(?:p)?info/) { # static shared data tally
	          print TRANS;
                  while (<W2C>) {
                    $w2cline++;
	            print TRANS;
		  ...
		  print TRANS $line;
                    ...
                    s/\bbupc__uint128_t\b/__uint128_t/g;
                    s/\bbupc__int128_t\b/__int128_t/g;
                    print TRANS;
                }
            }
            close (W2C);
    ...
    foreach my $basename (@toTranslate) {
        ...
        open(TRANS, ">$dst") or die "Could not open $dst for writting.";
        {
            ...
            while(<TRANSTMP>) {
                ...
                } elsif (/^\s*upcr_startup_(?:p)?shalloc_t\s+(?:_bupc_)?(?:p)?info.*?=\s*{(.*?)}/) { # static shared data tally
                    my $shalloc = $1;
                    #print "SHALLOC: $shalloc\n";
                    while ($shalloc =~ m/^\s*UPCRT_STARTUP_(?:P)?SHALLOC\s*\(\s*([A-Za-z0-9_]+)\s*,\s*([0-9]+)[ul\s]*,\s*([0-9]+)[ul\s]*,\s*([0-9]+)[ul\s]*(?:,\s*([0-9]+)[ul\s]*,\s*"([^"]*)")?\s*\)\s*,?(.*)$/i) {
                        #print "got one: $1 $2 $3 $4 $5 $6\n";
                        my $ident = "GASNETT_IDENT(UPCRI_IdentString_${file_mangled}_shareddata_$1,\n";
                        my $more = ($5 ? " $5 $6": "");
                        $ident .= " \"\$UPCRStaticSharedData: ($1) $2 $3 $4$more \$\");\n";
                        $extra_idents{$1} = $ident;
                        $shalloc = $7;
                    }
                }
            ...
            foreach my $ident (values %extra_idents) {
              print TRANS $ident;
            }
        }
        close(TRANS);
...
sub do_link
{
    ...
    my $extra_libs = ''; # unsed currently, but available for generalization
    my $cmd = $gmake_upcc
            . qq| UPCR_CONDUIT=$opt_network|
    ...
    $cmd .= " EXTRA_LIBS='$extra_libs'" if $extra_libs;
...
upcri_linkergenerated_tld_init(void)
{
    ...
    void * volatile _upcri_tmp; /* issue 96: prevent memcpy misoptimization by clang, triggered by our type-punning */

#if UPCRI_USING_TLD
  #define UPCR_TRANSLATOR_TLD(type, name, initval) _upcri_tmp = &name; memcpy(&_upcri_p->name, _upcri_tmp, sizeof(type));
  #include <upcr_translator_tld.h>
  #define UPCR_TLD_DEFINE(name, size, align) _upcri_tmp = &name; memcpy(&_upcri_p->name, _upcri_tmp, size);
  #include "global.tld"
#endif /* UPCRI_USING_TLD */
...
GASNETT_IDENT(UPCRI_IdentString_HeapSz, 
 "\$UPCRDefaultHeapSizes: UPC_SHARED_HEAP_OFFSET=0 UPC_SHARED_HEAP_SIZE=$opt_shared_heap \$");
...
EOF
  ...
  static void upcri_do_mcleanup(void) {
    UPCR_BEGIN_FUNCTION();
    gex_Rank_t ranks = gex_TM_QuerySize(upcri_tm);
    gex_Rank_t myrank = gex_TM_QueryRank(upcri_tm);
    if (ranks > 1) { /* attempt to chdir into a private per-process dir */
      char path[80];
      sprintf(path,"./gmon.out.%i", (int)myrank);
...
GASNETT_IDENT(__TV_s2s_disabled, "This UPC library lacks Totalview support.");
EOF
    }
