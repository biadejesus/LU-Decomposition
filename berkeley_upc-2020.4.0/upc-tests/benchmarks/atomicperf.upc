...
static void _report(int64_t timeus, const char *msg, int include_this_thread, int iters) {
  const char *threshmark = "";
  double myus = ((double)timeus)/iters;
  ...
  if (!MYTHREAD) {
    ...
    if (minus*iters < TIMER_THRESHOLD_US) {
      threshmark = "(*)";
      threshwarning = 1;
    }
    printf(" %-40s: Total=%6.3f sec  Avg/Min/Max=%6.3f %6.3f %6.3f us %s\n",
      msg, ((double)maxus)*iters/1000000, avgus, minus, maxus, threshmark);
    fflush(stdout);
  }
}
#define report(timeus, msg, include_this_thread) _report(timeus, msg, include_this_thread, giters)
...
int main(int argc, char **argv) {
  ...
  #define TEST_OP(desc, code) do {                      \
    upc_barrier;                                        \
    if (selected)                                       \
      for (i=0; i < MAX(10,iters/10); i++) { code; }    \
    upc_barrier;                                        \
    start = TIME();                                     \
    if (selected) {                                     \
      for (i=0; i < iters; i++) {                       \
  ...
  #define TEST(T, TM, OPS) do {                                                         \
    ZEROMSG("\n    --- Timing "#TM" atomics ---\n\n");                                  \
    int ad_iters = MAX(10,iters/100);                                                   \
    upc_atomicdomain_t **domarr = malloc(ad_iters*sizeof(upc_atomicdomain_t *));        \
    static shared T vals[THREADS];                                                      \
    shared T *ltarget = &vals[MYTHREAD];                                                \
    shared T *ptarget = &vals[partnerid];                                               \
    shared T *ztarget = &vals[0];                                                       \
    upc_barrier;  /* warmup */                                                          \
    for (i=0; i < ad_iters; i++) domarr[i] = upc_all_atomicdomain_alloc(TM, OPS, 0);    \
    for (i=0; i < ad_iters; i++) upc_all_atomicdomain_free(domarr[i]);                  \
    for (i=0; i < iters; i++) upc_atomic_isfast(TM, OPS, ptarget);                      \
    upc_barrier; /* alloc test */                                                       \
    start = TIME();                                                                     \
    for (i=0; i < ad_iters; i++) {                                                      \
      domarr[i] = upc_all_atomicdomain_alloc(TM, OPS, 0);                               \
    }                                                                                   \
    total = TIME() - start;                                                             \
    upc_barrier;                                                                        \
    _report(total, "upc_all_atomicdomain_alloc", 1, ad_iters);                          \
    ...
    total = TIME() - start;                                                             \
    _report(total, "upc_all_atomicdomain_free", 1, ad_iters);                           \
