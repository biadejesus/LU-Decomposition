...
extern void upcri_stats_finish(void (*stat_printf)(const char *, ...)) {
  ...
  stat_printf("");
  if (upcri_mynode == 0) { /* shared heap stats currently only supported on node 0 */
    char prefix[10];
    char buf[1024];
    snprintf(prefix,sizeof(prefix),"%i>  ", (int)upcri_mynode);
  ...
  for (i=0; i < upcri_mypthreads(); i++) {
    ...
    stat_printf(" *** UPC thread %i", (int)(i+upcri_1stthread(upcri_mynode)));
