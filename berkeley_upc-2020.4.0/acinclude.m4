...
if test x${upcri_did_aliases} != x1; then
 if test [$]# -gt 0; then 
  ...
  if test -n "$upcri_opt_version" ; then
    upcri_hello_shown=1
  elif ( test "$upcri_multiconf" = "1" || test "$upcri_help" = "1" || test "$upcri_reboot" = "1" ) \
