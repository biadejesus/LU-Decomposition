 ...
 */
	...
	!(UPCRI_UPC_PTHREADS || GASNET_PSHM)        && 0 /* disabled */
/* NOTE:
 * In GASNet-EX, a non-zero definition of GASNET_ALIGNED_SEGMENTS is only a
 * promise that gasnet_attach() will provide alignment, and NOT any guarantee
 * regarding gex_Segment_Attach (which we use).
 * So, we cannot rely upon it here.
 *
 * TODO-EX: restore/revise the SINGLE_ALIGNED_REGIONS logic using
 * offset-based addressing once supported in GASNet-EX.
 */
 ...
 * equal segments.  Happens always on SMP w/o PSHM.
 */
