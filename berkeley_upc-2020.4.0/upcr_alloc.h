...
/* for allocations before gasnet fully initialized */
...
// Bug3835: The malloc capture macros below must NOT be function-like macros,
// because when using the debug mallocator that would result in heap corruption 
// if the user directly creates a function pointer to any of these.
// This way at worst the user gets a compile error (due to the fact the expressions
// below are not l-values), which is easily resolved by creating their own one-line 
// wrapper around the call and using a pointer to that wrapper function.
#ifdef malloc
#undef malloc
#endif
#define malloc        (upcri_srcpos(), upcri_malloc)
#ifdef calloc
#undef calloc
#endif
#define calloc        (upcri_srcpos(), upcri_calloc)
#ifdef realloc
#undef realloc
#endif
#define realloc       (upcri_srcpos(), upcri_realloc)
#ifdef strdup
#undef strdup
#endif
#define strdup        (upcri_srcpos(), upcri_strdup)
#ifdef strndup
#undef strndup
#endif
#define strndup       (upcri_srcpos(), upcri_strndup)
#ifdef free
#undef free
#endif
#define free          (upcri_srcpos(), upcri_free)
