...
my @warning_blacklist = map { '^' . $_ . '$' } (
  ...
  'warning: \(.*?\)\s+failed to insert symbol \'.*?\' in the debug map\.',
  'warning: \(.*?\)\s+could not find object file symbol for symbol .*?',
  '',
);
...
# Create a bin directory for runs, if workdir is non-default
my $runjobs_script = undef;
if ($harness_run_path eq "$top_work_path/harness") {
  $runjobs_script = "$harness_run_path/runjobs";
} else {
  my $bindir = "$top_work_path/bin";
  mkdir($bindir, 0755) or &fatal("Could not create directory [$bindir]")
    unless -d "$bindir/.";
  foreach ('runjobs', 'runjobs.pl', 'harness_util.pl') {
    copy("$harness_run_path/$_", "$bindir/") or &fatal("Failed to copy $_ to $bindir/")
      unless -f "$bindir/$_";
  }
  $runjobs_script = "$bindir/runjobs";
  chmod +(0777 & ~umask), $runjobs_script;
}
...
$ENV{'HARNESS_LOGDIR'} = $logdir;
$ENV{'HARNESS_WORKDIR'} = $top_work_path;
$ENV{"AR"} = $compiler_spec{ar}; 
$ENV{"RANLIB"} = $compiler_spec{ranlib}; 
$ENV{'PERL'} = "$^X" unless $ENV{'PERL'};
$ENV{"UPC_HOME"} = $compiler_spec{upc_home}; 
$ENV{"UPCRUN_CMD"} = $compiler_spec{upcrun_command}; 
if (have_feature('runtime_upcr')) {
  ...
  $compiler_spec{cflags} = $makevars{"GASNET_CFLAGS"} . " " .
                   $makevars{"GASNET_MISC_CPPFLAGS"} if (!$compiler_spec{cflags});
  $compiler_spec{ld} = ($makevars{"GASNET_LD"}||'') if (!$compiler_spec{ld});
  ...
  while (<SIZES>) {
    ...
    if (m/^maxblocksz\s+(\d*)$/ && 
       # bug 3782: CUPC* and GUPC sometimes enforce an incorrect max blocksize for the configured phase bits, 
       # force a probe to find the actual largest usable block size
       have_feature('trans_bupc')) { 
  ...
  # Threading support details:
  # We want the '(no)tiopt' portion of the PAR build's GASNetConfig, if any.
  # We also want to know if '-uses-threads' support is present.
  sub get_gasnet_config($) {
    # returns GASNetConfig string for library corresonding to the provided
    # upcc arguments, or undef on error (such as unsupported args).
    my $args = shift;
    my $filename = "$sizes_file.tmp";
    my $cmd = "$upcc_with_flags -show-sizes -nopthreads $args > $filename 2>/dev/null";
    my $result = undef;
    &logit("querying GASNetConfig with '$args' ...");
    system($cmd);
    open (SIZES, $filename) || die "Failed to open $filename: $!\n";
    while (<SIZES>) {
      if (m/^GASNetConfig\s+(.*)$/) {
	$result = $1;
        last;
  ...
  my $gasnet_config_par = get_gasnet_config('-uses-threads');
  if (defined $gasnet_config_par) {
    # -uses-threads is PAR
    push @compiler_features, 'uses_threads_support';
  } elsif (",$gasnet_config," =~ m/,THREADMODEL=PAR,/) {
    # already PAR (-pthreads)
    $gasnet_config_par = $gasnet_config;
  } elsif (have_feature('pthread_support')) {
    $gasnet_config_par = get_gasnet_config('-pthreads');
  } else {
    # TODO: What to do when GASNet's par libs are built, but UPCR's are not?
    # This occurs (at least) when using gupc but lacking __thread support,
    # which is precisely one of the cases where tiopt is used.
    &logit("WARNING: Cannot query properties of the GASNet par library");
  }
  if (",$gasnet_config_par," =~ m/,tiopt,/) {
    push @compiler_features, 'par_uses_tiopt';
  }

  # TODO-EX: temporary lack of UPC collectives when using pthreads for UPC threads
  # This is a bit of a hack, since configure asserts the feature we must remove it
  if ($sysconf->{num_pthreads}) {
    @compiler_features = grep(!/^upc_collective$/,@compiler_features);
  }
  ...
  if (!$compiler_spec{ld}) { # default ld to cc
    $compiler_spec{ld} = $compiler_spec{cc};
    # and default to pickup C flags when defaulting ld
    $compiler_spec{ldflags} = $compiler_spec{cflags} if (!$compiler_spec{ldflags});
  }
}

{ # finalize features with command-line overrides
  my @tmp = (@compiler_features, @extra_features);
  @tmp = split(/,/,join(',',@tmp));
  @compiler_features = ();
  foreach my $feature (@tmp) {
    my ($not,$feat) = ($feature =~m/^(\!)?(.+)$/);
    @compiler_features = grep(!/^$feat$/,@compiler_features);
    push @compiler_features, $feature unless $not;
  }
}
...
if (!$compiler_spec{host_cc}) {
  ...
  $compiler_spec{host_ld} = $compiler_spec{ld};
...
foreach my $n ("cc","cflags","ld","ldflags","libs",
         "host_cc","host_cflags","host_ld","host_ldflags","host_libs") {
...
sub run_suite {
    ...
    my @tests = ();
    my $all_tests = defined($test_filter) && !(grep(/^[^!]/, @$test_filter)); # Start with all tests if filter is only negative
    TEST:
	...
	if (defined($test_filter)) {
          my $selected = $all_tests;
          foreach my $filterexpr (@$test_filter) {
            my ($not, $filter) = ($filterexpr =~ m/^(\!?)(.*)$/);
            if (($barecode =~ m/$filter/i) || 
                ("$suite\/$barecode" =~ m/$filter/i) ) { # match
              $selected = !$not;
            }
          }
	  if (!$selected) {
	...
	if (defined($excludefilter)) {
	    ...
	    my $expr = $conf->{RequireFeature};
	    $expr =~ s/^\s*//g;
	    $expr =~ s/\s*$//g;
	    if ($expr ne '') {
	      $expr =~ s/(\w)[,\s]+(\w)/$1 && $2/g; # backwards compatibility: comma/space delimiter on RequireFeature means AND
              $expr = " $expr ";
              $expr =~ s!([^a-zA-Z_\&'-])([a-zA-Z][a-zA-Z0-9_-]*)!$1(\&have_feature('\L$2\E'))!g;
              if (!feature_applicable($expr,$conf)) {
                &logit("Skipping test [$suite/$code] because it requires missing feature [$conf->{RequireFeature}]");
	        next TEST;
              }
	    }
        }
    	if (defined($conf->{ProhibitFeature})) {
	    my $expr = $conf->{ProhibitFeature};
	    $expr =~ s/^\s*//g;
	    $expr =~ s/\s*$//g;
	    if ($expr ne '') {
	      $expr =~ s/(\w)[,\s]+(\w)/$1 || $2/g; # backwards compatibility: comma/space delimiter on ProhibitFeature means OR
              $expr = " $expr ";
              $expr =~ s!([^a-zA-Z_\&'-])([a-zA-Z][a-zA-Z0-9_-]*)!$1(\&have_feature('\L$2\E'))!g;
              if (feature_applicable($expr,$conf)) {
                &logit("Skipping test [$suite/$code] because it prohibits feature [$conf->{ProhibitFeature}]");
	        next TEST;
              }
	    }
    	}
...
sub check_filter {
    ...
    if ($filter =~ m/^\^(.+)/) { # Implement NOT
      return !check_filter($1, $conf);
    }

    if ($filter =~ m/\|/) { # Implement OR
      foreach my $sub_filter (split('\|',$filter)) {
        return 1 if (check_filter($sub_filter, $conf));
      }
      return 0;
    }
...
sub get_runcmd {
    my @result = parse_conditionals($_[0], 'RunCmd', $_[1]);
    # RunCmd is not additive across inheritance - the last active value "wins"
    return $result[-1];
}
...
sub gen_lsf_qscript {
    ...
    printf $bf ("uname -a >> \$RUNLOG\n");
    printf $bf ("echo \"NODES\" >> \$RUNLOG\n");
    printf $bf ("if test -n \"\$LSB_DJOB_HOSTFILE\"; then\n");
    printf $bf ("  uniq < \$LSB_DJOB_HOSTFILE >> \$RUNLOG\n");
    printf $bf ("else\n");
...
sub gen_slurm_qscript {
    ...
    printf $bf ("#$sbatch -p %s\n", $q->{Q_name}) if exists($q->{Q_name});
    printf $bf ("#$sbatch --qos=%s\n", $q->{Q_qos}) if exists($q->{Q_qos});
    if ($q->{Q_noppn}) {
      &logit("WARNING: Q_noppn set, but ppn=".$rlist->{tpn}) if ($rlist->{tpn} != 1);
    } elsif ($tpnopt ne 'N/A') {
...
sub gen_cobalt_qscript {
    ...
    printf $bf ("test -n \"\$COBALT_NODEFILE\" && cat \$COBALT_NODEFILE >> \$RUNLOG\n");
    printf $bf ("\n");
...
sub submit_test {
    ...
    my $npthread = defined($conf->{Pthreads}) ? $conf->{Pthreads} : $num_pthreads;
    ...
    my $runcmd = get_runcmd($conf,$nthread);
...
sub compile_test {
    ...
    if ($bldcmd eq "upcc") {
	$upcflags .= " -pthreads -nolink-cache" if ($npthread > 0);
	if ($nolink) {
...
sub read_suite_config {
    ...
    # By default ignore KnownFailures in harness.conf if compiler.spec provides known_failures
    my $honor_known_failures = ! $compiler_spec{known_failures};
	...
	if (/^KnownFailureAnySpec/) {
	    # TODO: restrict placement in the file?
	    $honor_known_failures = 1;
	    next;
	}
	...
	if ($key eq "TestName") {
	    ...
	    &syntax("Invalid time value in line [$_] at [$path:$line]", $curspec) unless defined($val);
            ...
            next unless $honor_known_failures;
            dequote $val;
            parse_known($val,"[$path:$line]",$curspec);
            next;
	} elsif ($key eq "RequireFeature" || $key eq "ProhibitFeature") {
	    dequote $val;
	    if (exists($curspec->{$key}) && ($curspec == $default or $curspec->{$key} ne $default->{$key})) {
	   ...
	   $curspec->{BenchmarkResult} = $val; 
	} elsif (grep($key eq $_, qw/WarningFilter Flags MakeFlags AppArgs AppEnv RunCmd/)) {
	    ...
	    $val = $1;
	} elsif ($key eq "Pthreads") {
	   $curspec->{Pthreads} = $val if $num_pthreads; # Ignored in a non-pthreads harness
	}
...
sub filter_conf {
	...
	# hide $THREADS$ which is handled later in parse_conditionals
	$val =~ s/\$THREADS\$/\%\%THREADS\%\%/g;
	while ($val =~ /(\$(\w+)\$)/) {
	    my $str = $1;
	    my $sym = $2;
...
sub parse_args {
    ...
    sub parse_suite {
      my ($opt, $arg) = @_;
      if ($opt eq 'suite-order') {
        push @suite_order, $arg;
      } elsif ($opt eq 'suite') {
        push @suite, $arg;
      } else {
        $arg =~ s/,/,!/g;
    ...
    sub parse_feature {
      my ($opt, $arg) = @_;
      if ($opt eq 'add_feature') {
        push @extra_features, $arg;
      } else {
        $arg =~ s/,/,!/g;
        push @extra_features, ('!' . $arg);
      }
    ...
    my $status = GetOptions(
			    ...
			    'suite-order=s'  => \&parse_suite,
			    'test=s'         => \&parse_test,
			    'not_test=s'     => \&parse_test,
			    ...
			    'keep!'          => \$keep_binary,
	                    ...
	                    'add_feature=s'  => \&parse_feature,
	                    'del_feature=s'  => \&parse_feature,
    ...
    @compiler_features = split(/[, ]+/,$compiler_featurelist);
    ...
    push @compiler_features, "trans_g" if $upcc_debug;
    ...
    @suite_order = split(/,/,join(',',@suite_order));
    @test = split(/,/,join(',',@test));
    foreach my $thistest (@test) {
	if (!($thistest =~ m/^!/) && 
            $thistest =~ m/^\^?(.*)\/([^\/]+)$/) { # contains a suite spec, select the suite
	  push @suite, $1;
	}
    }
    ...
    # sort the suites
    my @suite_prefix = ();
    my @suite_suffix = @{$sysconf->{testsuites}};
    sort { lc($a) cmp lc($b) } @suite_suffix;
    foreach my $pat (@suite_order) {
      print "Considering suite-order pat [$pat]\n" if ($debug);
      my @next_suffix = ();
      map { m/$pat$/ ? push @suite_prefix, $_ : push @next_suffix, $_ } @suite_suffix;
      @suite_suffix = @next_suffix;
      print "PREFIX: ".join(',',@suite_prefix)."\n" if ($debug);
    }
    $sysconf->{testsuites} = \@suite_prefix;
    push @{$sysconf->{testsuites}}, @suite_suffix;
	...
	$logdir = "$sysconf->{logroot}/$timestamp";
    } elsif (defined($wpath)) {
	$logdir = "$wpath/logdir";
    } else {
	$logdir = "$harness_run_path/logroot/$timestamp";
    }
    ...
    foreach my $filter (@includefilter) {
      foreach my $elem (split('\|',$filter)) {
        (my $tmp = $elem) =~ s/^(known-[a-z-]+)\+$/$1/; # strip trailing '+', if any
        $tmp =~ s/^\^//; # strip leading '^', if any
        unless (grep($_ eq $tmp,@VALID_FILTERS) || ($elem =~ m/^\^?(require|prohibit)-feature:.+/)) {
...
using environment variable \$HARNESS_FLAGS):
   ...
   -repo=name         Specify name of the accounting repository.
   ...
   -add_feature=str   Optional comma-delimited list of features to append to the value of -features or
                      the automatically determined equivalent. This option may be repeated multiple times.
   -del_feature=str   Optional comma-delimited list of features to remove from the feature list.
                      This option may be repeated multiple times, and the last -[add|del]_feature
		      directive mentioning a given feature takes precedence.
   -g,-O,-opt,-profile,-profile-local                 
   ...
   -include=string    Specify a condition that a test must satisfy if it is
                      ...
                      suffixed with a '+' to ignore the features list.  The
                      argument may use '|' to separate conditions to be ORed.
                      Each condition may be prefixed with '^' to negate it.
                      The argument may be a comma separated list of expressions,
                      and this option may also be repeated.  In these cases all
                      expressions must evaluate true for a test to be compiled.
   ...
   -not_test=name     Specify a comma-separated list of tests to exclude.
                      For both -test and -not_test, the argument is actually a 
                      (default unanchored) regex that needs to match the bare testname 
                      or suitename/testname. The lists from -test and -not_test
                      are processed in the order given, where the last match "wins".
   -suite=name_suff   Specify the name of test suite(s) to run.  You need only
   ...
   -suite-order=name  Specify an order prefix for the selected suites.  This does not affect the 
                      suite selection, only the order they are compiled and added to runlists.
                      As with -suite, you need only specify the unique trailing part of the name,
                      and the argument may be a comma separated list or this option may be repeated.
                      Any matching suites are run first, in the specified order, and any 
                      remaining suites follow in alphabetic order.
...
EOF
    ...
    foreach my $test (sort { lc($a) cmp lc($b) } @display_suites) {
	print("\t\t\t\t$test\n");
    }
    exit(!!@ARGV);
}
