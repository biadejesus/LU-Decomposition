 ...
 */
  ...
  #define UPCRI_TRACE_MEMSET(name,dest,val,nbytes) \
    if (!upcri_trace_suppresslocal)                   \
      UPCRI_TRACE_PRINTF_NOPOS(("%s: sz = %6llu", (name), (unsigned long long)(nbytes)))
  ...
  #define UPCRI_TRACE_MEMSET(namestr,dest,val,nbytes)   ((void)0)
 ...
 * some dependencies that were undesirable for Totalview support, so they
 * were moved here.
 */
#if GASNET_CONDUIT_SMP && !GASNET_PSHM
  /* special case - conduit smp only supports one node, so skip the lookup costs */
  #define upcri_shared_nodeof(sptr)  (upcri_checkvalid_shared(sptr), ((gex_Rank_t)0))
  #define upcri_pshared_nodeof(sptr) (upcri_checkvalid_pshared(sptr), ((gex_Rank_t)0))
#else
 ...
 */
  ...
  #define upcri_s_nodeof(s)	((gex_Rank_t)0)
  #define upcri_p_nodeof(p)	((gex_Rank_t)0)
  ...
  #define UPCRI_PASS_GAS() \
	  GASNET_POST_THREADINFO(upcri_mypthreadinfo()->mygasnet_tinfo)
 #if 0
 ...
 * NOTE: These are *not* documented GASNet-Tools interfaces.
 */

#define UPCRI_ALIGNED_MEMCPY(dest, src, nbytes) \
       GASNETT_FAST_ALIGNED_MEMCPY(dest, src, nbytes)

#define UPCRI_UNALIGNED_MEMCPY(dest, src, nbytes) do { \
    void * _uum_dst = (dest);                                           \
    void const * _uum_src = (src);                                      \
    size_t _uum_nbytes = (nbytes);                                      \
    if_pt (_uum_nbytes && (_uum_dst != _uum_src))                       \
        GASNETT_FAST_UNALIGNED_MEMCPY(_uum_dst, _uum_src, _uum_nbytes); \
  } while (0);
...
GASNETT_INLINE(upcr_shared_to_processlocal)
...
GASNETT_INLINE(upcr_pshared_to_processlocal)
 ...
 * GEX_EVENT_INVALID, then the required RMB() will be performed in the
 ...
 * TODO: replace '#if 0' with list of conduits w/ an in-order guarantee
 */
#if 0 && defined(UPCRI_STRICT_HOOK_ENABLED)
		 ...
		 size_t nbytes, int isstrict UPCRI_PT_ARG)
{
	...
	UPCRI_TRACE_PUT(PUT_LOCAL, upcri_mynode,
	...
	upcri_strict_mb(isstrict);
        ...
        upcri_strict_wmb(isstrict);
        gex_RMA_PutBlocking(upcri_tm, upcri_s_nodeof(dest),
		  ...
		  size_t nbytes, int isstrict UPCRI_PT_ARG)
{
	...
	upcri_strict_mb(isstrict);
        ...
        upcri_strict_wmb(isstrict);
        gex_RMA_PutBlocking(upcri_tm, upcri_p_nodeof(dest),
		 ...
		 size_t nbytes, int isstrict UPCRI_PT_ARG)
{
	...
	UPCRI_TRACE_GET(GET_LOCAL, dest, upcri_mynode,
	...
	upcri_strict_wmb(isstrict);
        gex_RMA_GetBlocking(upcri_tm, dest, upcri_s_nodeof(src),
		  ...
		  size_t nbytes, int isstrict UPCRI_PT_ARG)
{
	...
	upcri_strict_wmb(isstrict);
        gex_RMA_GetBlocking(upcri_tm, dest, upcri_p_nodeof(src),
 ...
 * function callers.
 */
typedef gex_Event_t upcr_handle_t;

#define UPCR_INVALID_HANDLE GEX_EVENT_INVALID
		    ...
		    size_t nbytes, int isstrict UPCRI_PT_ARG)
{
    ...
    if (local) upcri_pevt_putnb_local(s,{
        upcri_strict_wmb(isstrict);
	UPCRI_TRACE_PUT(PUT_NB_LOCAL, upcri_mynode,
			upcri_s2localoff(local, dest, destoffset),
			src, nbytes);
	UPCRI_ALIGNED_MEMCPY(upcri_s2localoff(local, dest, destoffset),
			     src, nbytes);
	...
	h = UPCR_INVALID_HANDLE;
        ...
        upcri_strict_wmb(isstrict);
        h = gex_RMA_PutNB(upcri_tm, upcri_s_nodeof(dest),
                            upcri_shared_to_remote_off(dest, destoffset),
                            (void *)src, nbytes, GEX_EVENT_NOW, 0);
		    ...
		    size_t nbytes, int isstrict UPCRI_PT_ARG)
{
	...
	UPCRI_TRACE_GET(GET_NB_LOCAL, dest, upcri_mynode,
			upcri_s2localoff(local, src, srcoffset), nbytes);
	upcri_strict_mb(isstrict);
	UPCRI_ALIGNED_MEMCPY(dest, upcri_s2localoff(local, src, srcoffset),
			     nbytes);
	...
	upcri_strict_wmb(isstrict);
        h = gex_RMA_GetNB(upcri_tm, dest, upcri_s_nodeof(src),
                            upcri_shared_to_remote_off(src, srcoffset),
                            nbytes, 0);
	if_pf (h == UPCR_INVALID_HANDLE)
	    upcri_strict_rmb(isstrict);
		     ...
		     size_t nbytes, int isstrict UPCRI_PT_ARG)
{
	...
	h = UPCR_INVALID_HANDLE;
        ...
        upcri_strict_wmb(isstrict);
        h = gex_RMA_PutNB(upcri_tm, upcri_p_nodeof(dest),
                            upcri_pshared_to_remote_off(dest, destoffset),
                            (void *)src, nbytes, GEX_EVENT_NOW, 0);
	if_pf (h == UPCR_INVALID_HANDLE)
	    upcri_strict_rmb(isstrict);
		     ...
		     size_t nbytes, int isstrict UPCRI_PT_ARG)
{
	...
	upcri_strict_wmb(isstrict);
        h = gex_RMA_GetNB(upcri_tm, dest,upcri_p_nodeof(src),
                            upcri_pshared_to_remote_off(src, srcoffset),
                            nbytes, 0);
	if_pf (h == UPCR_INVALID_HANDLE)
	    upcri_strict_rmb(isstrict);
...
int _upcr_try_syncnb(upcr_handle_t handle UPCRI_PT_ARG_IFINST) {
  ...
  upcri_pevt(GASP_BUPC_NB_TRYSYNC,{},{
    gasnet_AMPoll(); // TODO-EX: needed?
    retval = (gex_Event_Test(handle) == GASNET_OK);
  });
  #undef UPCRI_PEVT_ARGS
  return retval;
}
...
{
    gex_Event_WaitAll(phandles, numhandles, 0);
}
...
{
    gasnet_AMPoll(); // TODO-EX: needed?
    return (gex_Event_TestAll(phandles, numhandles, 0) == GASNET_OK);
}
...
{
    gex_Event_WaitSome(phandles, numhandles, 0);
}
...
{
    gasnet_AMPoll(); // TODO-EX: needed?
    return (gex_Event_TestSome(phandles, numhandles, 0) == GASNET_OK);
}
		     ...
		     size_t nbytes UPCRI_PT_ARG)
{
    ...
    if (local) upcri_pevt_putnb_local(s,{
	UPCRI_TRACE_PUT(PUT_NBI_LOCAL, upcri_mynode,
			upcri_s2localoff(local, dest, destoffset),
			src, nbytes);
	UPCRI_ALIGNED_MEMCPY(upcri_s2localoff(local, dest, destoffset),
			     src, nbytes);
    }); else upcri_pevt_putnb(s,{
        gex_RMA_PutNBI(upcri_tm, upcri_s_nodeof(dest),
                         upcri_shared_to_remote_off(dest, destoffset), (void *)src,
                         nbytes, GEX_EVENT_NOW, 0);
    });
    #undef UPCRI_PEVT_ARGS
}
		     ...
		     size_t nbytes UPCRI_PT_ARG)
{
    ...
    if (local) upcri_pevt_getnb_local(s,{
	UPCRI_TRACE_GET(GET_NBI_LOCAL, dest, upcri_mynode,
			upcri_s2localoff(local, src, srcoffset), nbytes);
	UPCRI_ALIGNED_MEMCPY(dest, upcri_s2localoff(local, src, srcoffset),
			     nbytes);
    }); else upcri_pevt_getnb(s,{
        gex_RMA_GetNBI(upcri_tm, dest, upcri_s_nodeof(src),
                         upcri_shared_to_remote_off(src, srcoffset),
                         nbytes, 0);
    });
    #undef UPCRI_PEVT_ARGS
		      ...
		      size_t nbytes UPCRI_PT_ARG)
{
	...
	UPCRI_ALIGNED_MEMCPY(upcri_p2localoff(local, dest, destoffset),
        ...
        gex_RMA_PutNBI(upcri_tm, upcri_p_nodeof(dest),
                         upcri_pshared_to_remote_off(dest, destoffset), (void *)src,
                         nbytes, GEX_EVENT_NOW, 0);
    });
    #undef UPCRI_PEVT_ARGS
}
		      ...
		      size_t nbytes UPCRI_PT_ARG)
{
	...
	UPCRI_ALIGNED_MEMCPY(dest, upcri_p2localoff(local, src, srcoffset),
        ...
        gex_RMA_GetNBI(upcri_tm, dest, upcri_p_nodeof(src),
                         upcri_pshared_to_remote_off(src, srcoffset),
                         nbytes, 0);
    });
    #undef UPCRI_PEVT_ARGS
...
int _upcr_try_syncnbi_gets(UPCRI_PT_ARG_ALONE) {
  ...
  upcri_pevt(GASP_BUPC_NB_TRYSYNC,{},{
    gasnet_AMPoll(); // TODO-EX: needed?
    result = (gex_NBI_Test(GEX_EC_GET,0) == GASNET_OK);
  });
  #undef UPCRI_PEVT_ARGS
  return result;
}
...
int _upcr_try_syncnbi_puts(UPCRI_PT_ARG_ALONE) {
  ...
  upcri_pevt(GASP_BUPC_NB_TRYSYNC,{},{
    gasnet_AMPoll(); // TODO-EX: needed?
    result = (gex_NBI_Test(GEX_EC_PUT,0) == GASNET_OK);
  });
  #undef UPCRI_PEVT_ARGS
  return result;
}
...
int _upcr_try_syncnbi_all(UPCRI_PT_ARG_ALONE) {
  ...
  upcri_pevt(GASP_BUPC_NB_TRYSYNC,{},{
    gasnet_AMPoll(); // TODO-EX: needed?
    result = (gex_NBI_Test(GEX_EC_ALL,0) == GASNET_OK);
  });
  #undef UPCRI_PEVT_ARGS
  return result;
}
 ...
 */
// While we previously used gex_RMA_Value_t, we now use uintptr_t since the
// former is now uint64_t on all platforms (even ILP32 ones).  Note there is
// an implicit assumtion here that `sizeof(uintptr_t) == sizeof(void*)`, even
// though the C spec does not strictly require this to be the case.
//    - PHH June, 2019
typedef uintptr_t upcr_register_value_t;
#define SIZEOF_UPCR_REGISTER_VALUE_T SIZEOF_VOID_P
		     ...
		     int isstrict UPCRI_PT_ARG)
{
    ...
    if (local) upcri_pevt_put_local(s,{
        upcri_strict_wmb(isstrict);
	UPCRI_TRACE_PUT(PUT_VAL_LOCAL, upcri_mynode,
			upcri_s2localoff(local, dest, destoffset),
			&value, nbytes);
	...
	upcri_strict_mb(isstrict);
        ...
        upcri_strict_wmb(isstrict);
        gex_RMA_PutBlockingVal(upcri_tm, upcri_s_nodeof(dest),
                         upcri_shared_to_remote_off(dest, destoffset),
                         value, nbytes, 0);
	upcri_strict_rmb(isstrict);
    });
			...
			int isstrict UPCRI_PT_ARG)
{
    ...
    if (local) upcri_pevt_putnb_local(s,{
        upcri_strict_wmb(isstrict);
	UPCRI_TRACE_PUT(PUT_NB_VAL_LOCAL, upcri_mynode,
			upcri_s2localoff(local, dest, destoffset),
			&value, nbytes);
	UPCRI_VALUE_ASSIGN(upcri_s2localoff(local, dest, destoffset),
			     value, nbytes);
	...
	h = UPCR_INVALID_HANDLE;
        ...
        upcri_strict_wmb(isstrict);
        h = gex_RMA_PutNBVal(upcri_tm, upcri_s_nodeof(dest),
                                upcri_shared_to_remote_off(dest, destoffset),
                                value, nbytes, 0);
	if_pf (h == UPCR_INVALID_HANDLE)
	    upcri_strict_rmb(isstrict);
			 ...
			 UPCRI_PT_ARG)
{
    ...
    if (local) upcri_pevt_putnb_local(s,{
	UPCRI_TRACE_PUT(PUT_NBI_VAL_LOCAL, upcri_mynode,
			upcri_s2localoff(local, dest, destoffset),
			&value, nbytes);
	UPCRI_VALUE_ASSIGN(upcri_s2localoff(local, dest, destoffset),
			     value, nbytes);
    }); else upcri_pevt_putnb(s,{
	gex_RMA_PutNBIVal(upcri_tm, upcri_s_nodeof(dest),
			     upcri_shared_to_remote_off(dest, destoffset),
			     value, nbytes, 0);
    });
    #undef UPCRI_PEVT_ARGS
		      ...
		      int isstrict UPCRI_PT_ARG)
{
	...
	upcri_strict_mb(isstrict);
        ...
        upcri_strict_wmb(isstrict);
        gex_RMA_PutBlockingVal(upcri_tm, upcri_p_nodeof(dest),
                         upcri_pshared_to_remote_off(dest, destoffset),
                         value, nbytes, 0);
	upcri_strict_rmb(isstrict);
    });
			 ...
			 int isstrict UPCRI_PT_ARG)
{
	...
	h = UPCR_INVALID_HANDLE;
        ...
        upcri_strict_wmb(isstrict);
        h = gex_RMA_PutNBVal(upcri_tm, upcri_p_nodeof(dest),
                                upcri_pshared_to_remote_off(dest, destoffset),
                                value, nbytes, 0);
	if_pf (h == UPCR_INVALID_HANDLE)
	    upcri_strict_rmb(isstrict);
			  ...
			  UPCRI_PT_ARG)
{
	...
	UPCRI_VALUE_ASSIGN(upcri_p2localoff(local, dest, destoffset),
        ...
        gex_RMA_PutNBIVal(upcri_tm, upcri_p_nodeof(dest),
                             upcri_pshared_to_remote_off(dest, destoffset),
			     value, nbytes, 0);
    });
    #undef UPCRI_PEVT_ARGS
		     ...
		     size_t nbytes, int isstrict UPCRI_PT_ARG)
{
    ...
    if (local) upcri_pevt_get_local(s,{
	UPCRI_TRACE_GET(GET_VAL_LOCAL, NULL, upcri_mynode,
			upcri_s2localoff(local, src, srcoffset), nbytes);
	upcri_strict_mb(isstrict);
	...
	upcri_strict_wmb(isstrict);
        retval = gex_RMA_GetBlockingVal(upcri_tm, upcri_s_nodeof(src),
                                  upcri_shared_to_remote_off(src, srcoffset), nbytes, 0);
        upcri_strict_rmb(isstrict);
    });
    #undef UPCRI_PEVT_ARGS
    return retval;
		      ...
		      size_t nbytes, int isstrict UPCRI_PT_ARG)
{
	...
	upcri_strict_wmb(isstrict);
        retval = gex_RMA_GetBlockingVal(upcri_tm, upcri_p_nodeof(src),
                                  upcri_pshared_to_remote_off(src, srcoffset), nbytes, 0);
        upcri_strict_rmb(isstrict);
    });
    #undef UPCRI_PEVT_ARGS
    return retval;
...
typedef void* upcr_valget_handle_t;
			  ...
			  int isstrict UPCRI_PT_ARG)
{
	...
	upcri_strict_wmb(isstrict);
        gex_RMA_GetBlocking(upcri_tm, &tmp, upcri_s_nodeof(src),
                     upcri_shared_to_remote_off(src, srcoffset),
                     sizeof(float), 0);
	retval = tmp;
        upcri_strict_rmb(isstrict);
			   ...
			   int isstrict UPCRI_PT_ARG)
{
	...
	float tmp; /* TODO: try to eliminate this stack temporary */
	upcri_strict_wmb(isstrict);
        gex_RMA_GetBlocking(upcri_tm, &tmp, upcri_p_nodeof(src),
                     upcri_pshared_to_remote_off(src, srcoffset),
                     sizeof(float), 0);
	retval = tmp;
        upcri_strict_rmb(isstrict);
...
_upcr_memget(void *dst, upcr_shared_ptr_t src, size_t nbytes UPCRI_PT_ARG)
{
    ...
    if (local) {
	UPCRI_TRACE_GET(GET_BULK_LOCAL, dst, upcri_mynode,
			upcri_s2local(local, src), nbytes);
	UPCRI_UNALIGNED_MEMCPY(dst, upcri_s2local(local, src), nbytes);
    } else {
        gex_RMA_GetBlocking(upcri_tm, dst, upcri_s_nodeof(src),
                     upcri_shared_to_remote(src), nbytes, 0);
    }
...
_upcr_memput(upcr_shared_ptr_t dst, const void *src, size_t nbytes UPCRI_PT_ARG)
{
    ...
    if (local) {
	UPCRI_TRACE_PUT(PUT_BULK_LOCAL, upcri_mynode,
			upcri_s2local(local, dst), src, nbytes);
	UPCRI_UNALIGNED_MEMCPY(upcri_s2local(local, dst), src, nbytes);
    } else {
#if defined(UPCRI_DO_MEMPUT_NBI)
        gex_RMA_PutNBI(upcri_tm, upcri_s_nodeof(dst), upcri_shared_to_remote(dst),
                         (void *)src, nbytes, GEX_EVENT_NOW, 0);
#else
        gex_RMA_PutBlocking(upcri_tm, upcri_s_nodeof(dst), upcri_shared_to_remote(dst),
                     (void *)src, nbytes, 0);
#endif
    }
...
/*
 * Sends an active message that does memset on remote node.
 *
 * Since we can't know when the memset will be completed except by waiting, the
 * 'nonblocking' version actually blocks.
 *
 * TODO-EX: can allocate handles and make a true non-blocking version.
 */
GASNETT_INLINE(upcri_do_remote_memset) GASNETT_WARN_UNUSED_RESULT
upcr_handle_t
upcri_do_remote_memset(gex_Rank_t node, void * dst,
		       int value, size_t nbytes)
{
    ...
    gex_AM_RequestShort(upcri_tm, node,
			UPCRI_HANDLER_ID(upcri_remote_memset_SRequest), 0,
			UPCRI_SEND_PTR(dst),
			value,
...
enum {
    upcri_memset_type_block = 0,
    upcri_memset_type_nb,
    upcri_memset_type_nbi
};

#define upcr_do_memset(dst, value, nbytes, memset_type) \
       _upcr_do_memset(dst, value, nbytes, memset_type UPCRI_PT_PASS)

extern upcr_handle_t _upcr_do_memset(upcr_shared_ptr_t dst, int value,
				     size_t nbytes, int memset_type UPCRI_PT_ARG);
...
_upcr_memset(upcr_shared_ptr_t dst, int c, size_t nbytes UPCRI_PT_ARG)
{
    ...
    (void) upcr_do_memset(dst, c, nbytes, upcri_memset_type_block);
...
_upcr_nb_memget(void *dst, upcr_shared_ptr_t src, size_t nbytes UPCRI_PT_ARG)
{
    ...
    if (local) {
	UPCRI_TRACE_GET(GET_NB_BULK_LOCAL, dst, upcri_mynode,
			 upcri_s2local(local, src), nbytes);
	UPCRI_UNALIGNED_MEMCPY(dst, upcri_s2local(local, src), nbytes);
	h = UPCR_INVALID_HANDLE;
    } else {
        h = gex_RMA_GetNB(upcri_tm, dst, upcri_s_nodeof(src),
                            upcri_shared_to_remote(src), nbytes, 0);
    }
...
_upcr_nb_memput(upcr_shared_ptr_t dst, const void *src, size_t nbytes UPCRI_PT_ARG)
{
    ...
    if (local) {
	UPCRI_TRACE_PUT(PUT_NB_BULK_LOCAL, upcri_mynode,
			upcri_s2local(local, dst), src, nbytes);
	UPCRI_UNALIGNED_MEMCPY(upcri_s2local(local, dst), src, nbytes);
	h = UPCR_INVALID_HANDLE;
    } else {
        h = gex_RMA_PutNB(upcri_tm, upcri_s_nodeof(dst),
                            upcri_shared_to_remote(dst), (void *)src,
...
{
    return upcr_do_memset(dst, c, nbytes, upcri_memset_type_nb);
}
...
_upcr_nbi_memget(void *dst, upcr_shared_ptr_t src, size_t nbytes UPCRI_PT_ARG)
{
    ...
    upcri_pevt_start(GASP_BUPC_NB_MEMGET_INIT);
    if (local) {
	UPCRI_TRACE_GET(GET_NBI_BULK_LOCAL, dst, upcri_mynode,
			upcri_s2local(local, src), nbytes);
	UPCRI_UNALIGNED_MEMCPY(dst, upcri_s2local(local, src), nbytes);
    } else {
        gex_RMA_GetNBI(upcri_tm, dst, upcri_s_nodeof(src),
                         upcri_shared_to_remote(src), nbytes, 0);
    }
    upcri_pevt_end(GASP_BUPC_NB_MEMGET_INIT);
    #undef UPCRI_PEVT_ARGS
...
_upcr_nbi_memput(upcr_shared_ptr_t dst, const void *src, size_t nbytes UPCRI_PT_ARG)
{
    ...
    upcri_pevt_start(GASP_BUPC_NB_MEMPUT_INIT);
    if (local) {
	UPCRI_TRACE_PUT(PUT_NBI_BULK_LOCAL, upcri_mynode,
			upcri_s2local(local, dst), src, nbytes);
	UPCRI_UNALIGNED_MEMCPY(upcri_s2local(local, dst), src, nbytes);
    } else {
        gex_RMA_PutNBI(upcri_tm, upcri_s_nodeof(dst),
                         upcri_shared_to_remote(dst), (void *)src,
                         nbytes, GEX_EVENT_DEFER, 0);
    }
    upcri_pevt_end(GASP_BUPC_NB_MEMPUT_INIT);
...
{
    (void) upcr_do_memset(dst, c, nbytes, upcri_memset_type_nbi);
}
...
upcr_handle_t
upcri_do_remote_memcpy(gex_Rank_t node, void * dst, void * src,
		       size_t nbytes)
{	    
    volatile int trigger = UPCRI_REQUEST_BLOCKED;
    volatile int *ptrigger = &trigger;
    ...
    /* Send nbytes as ptr, since size_t may be 64 bits */
    gex_AM_RequestShort(upcri_tm, node,
			UPCRI_HANDLER_ID(upcri_remote_memcpy_SRequest), 0,
			UPCRI_SEND_PTR(dst),
			UPCRI_SEND_PTR(src),
			UPCRI_SEND_PTR(nbytes),
			UPCRI_SEND_PTR(ptrigger));
