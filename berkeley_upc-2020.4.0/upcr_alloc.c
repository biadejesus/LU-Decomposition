...
uintptr_t upcri_localheap_initsz  = (8192 * 4);
uintptr_t upcri_globalheap_initsz = (8192 * 4);

#define UPCRI_MEGABYTE (1024*1024)
 ...
 */
static gex_HSL_t heapsizes_lock = GEX_HSL_INITIALIZER; 
...
upcri_expand_global_heapsize(uintptr_t morebytes, upcri_alloccaller_t caller)
{
	...
	/* Out of shared memory */
	gex_HSL_Unlock(&heapsizes_lock);
	gex_HSL_Unlock(&global_heap.lock);
...
void upcri_init_heaps(void *start, uintptr_t len)
{
    ...
    /* UPC_FIRSTTOUCH: dynamically perform first touch on shared heap memory to help
       ...
       physical memory allocation (eg Altix, Origin). 

       Enabled by default with UPCRI_SHARED_THREADS, ie whenever a UPC thread may
       have hardware load/store access to the shared segment of a peer (via pthread or PSHM).
       This default specifically ignores the actual co-location status at runtime, 
       since that can be a node-specific property that varies across the job 
       (and the all_alloc first touch implementation currently assumes the setting to be single-valued).
     ...
     */
    upcri_firsttouch = gasnett_getenv_yesno_withdefault("UPC_FIRSTTOUCH", UPCRI_SHARED_THREADS);
	...
	largest_local_heapsize = upcri_localheap_initsz;
    ...
    if (upcr_mythread() == 0) {
	global_heap.size = upcri_globalheap_initsz;
	gex_HSL_Init(&global_heap.lock);
#ifdef UPCR_GLOBALHEAP_GROWS_UP
...
#endif
    mylocalheap->size = upcri_localheap_initsz;
    mylocalheap->heap = upcra_sharedlocal_init(start, mylocalheap->size);
    gex_HSL_Init(&mylocalheap->lock);
            ...
            upcri_alloccaller_t caller)
{
	...
	if (neededlheapsz > largest_local_heapsize) {
		 ...
		 */
		gex_HSL_Unlock(&heapsizes_lock);
		gex_HSL_Unlock(&lh->lock);

		/* pass size_t's as pointers, since may be 64 bits */
		gex_AM_RequestShort(upcri_tm, 0,
				    UPCRI_HANDLER_ID(upcri_expand_local_SRequest), 0,
    ...
    return upcr_local_to_shared_withphase(MEMCHECK_POST_ALLOC(ptr, nbytes), 0, 
				upcri_1stthread(upcri_mynode) + pthread);
}
...
void upcri_expand_local_SRequest(gex_Token_t token,
				 void *neededlheapsz,
				 void *request_sz,
				 void *trigger_addr,
                                 gex_AM_Arg_t caller)
{
    ...
    gex_AM_ReplyShort(token,
			    UPCRI_HANDLER_ID(upcri_expand_local_SReply), 0,
			    UPCRI_SEND_PTR(newlargest),
			    UPCRI_SEND_PTR(trigger_addr));
}

void upcri_expand_local_SReply(gex_Token_t token,
			       void *newlargestlh,
			       void *trigger_addr)
{
    gex_HSL_Lock(&heapsizes_lock);
 ...
 * cached copy
 */
void upcri_global_alloc_SRequest(gex_Token_t token,
				 void *nblocks, 
				 void *blocksz, 
				 void *result_addr,
				 void *trigger_addr,
                                 gex_AM_Arg_t caller)
{
    ...
    upcri_assert(sizeof(upcr_shared_ptr_t) < gex_AM_LUBRequestMedium());
    ...
    gex_AM_ReplyMedium(token,
			 UPCRI_HANDLER_ID(upcri_global_alloc_MReply),
			 &out, sizeof(out), GEX_EVENT_NOW, 0,
 ...
 */
void upcri_global_alloc_MReply(gex_Token_t token,
			       void *msg, size_t len, 
...
_upcr_global_alloc(size_t nblocks, size_t blocksz UPCRI_PT_ARG)
{
	...
	gex_AM_RequestShort(upcri_tm, 0,
				UPCRI_HANDLER_ID(upcri_global_alloc_SRequest), 0,
...
upcri_do_local_free(upcr_shared_ptr_t sptr)
{
    ...
    upcri_assert(upcri_shared_nodeof(sptr) == upcri_mynode);
...
{
    gex_Rank_t sptrnode = upcri_shared_nodeof(sptr);
    ...
    } else if (sptrnode == upcri_mynode) {
	upcri_do_local_free(sptr);
    } else {
	gex_AM_RequestMedium0(upcri_tm, sptrnode,
		       UPCRI_HANDLER_ID(upcri_free_MRequest),
		       &sptr, sizeof(sptr), GEX_EVENT_NOW, 0);
    }
...
void 
upcri_free_MRequest(gex_Token_t token, void * addr, size_t len)
{
    upcri_assert(len == sizeof(upcr_shared_ptr_t));
