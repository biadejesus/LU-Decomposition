...
#else
  /* Can assume tid == node, so tids are obtained from upcri_mynode */
  #define UPCR_LOCK_TID_OF_RCVR(var) \
      upcr_thread_t var = upcri_mynode;
...
/* low-level locks controlling access the internals of the MCS lock */
  ...
  #undef UPCRI_LOCKS_PSHARED
  #define upcri_lllocksystem_init() ((void)0)
  typedef gex_HSL_t upcri_lllock_t;
  #define upcri_lllock_init(_lp)    gex_HSL_Init(_lp)
  #define upcri_lllock_fini(_lp)    gex_HSL_Destroy(_lp)
  #define upcri_lllock_acquire(_lp) gex_HSL_Lock(_lp)
  #define upcri_lllock_release(_lp) gex_HSL_Unlock(_lp)
#else
...
#else
  #define upcri_lock_pshared_addr(_n,_t,_a) (((_n) == upcri_mynode) ? (_a) : NULL)
#endif
/* -------------------------------------------------------------------------- */
#if UPCRI_AMLOCKS
...
#endif
/* -------------------------------------------------------------------------- */
void upcri_SRQ_lockdestroy(gex_Token_t token,
                           gex_AM_Arg_t freer_threadid
                           UPCR_LOCK_TID_ARG(gex_AM_Arg_t lock_threadid),
                           void *_lock) {
#if !UPCRI_AMLOCKS
    ...
    gex_AM_ReplyShort(token,
                      UPCRI_HANDLER_ID(upcri_SRP_lockreply), 0
...
#endif
void upcri_SRQ_freeheldlock(gex_Token_t token, void *_tail_addr) {
  #if UPCRI_AMLOCKS
...
void _upcr_do_lock_free(upcr_shared_ptr_t lockptr UPCRI_PT_ARG) {
    ...
    gex_Rank_t node = upcri_thread_to_node(lock_threadaff);
  ...
  UPCRI_LOCKC_DESTROY(lockptr, lockaddr);
    ...
    if (node == upcri_mynode) {
      ...
      gex_AM_RequestShort(upcri_tm, node,
	   UPCRI_HANDLER_ID(upcri_SRQ_lockdestroy), 0,
           upcr_mythread()
    ...
    if (sli->reply_thread != UPCRI_INVALID_THREAD) {
        ...
        local_addr = (upcri_thread_to_node(tail_thread) == upcri_mynode)
                         ? tail_addr : NULL;
      #else
        ...
        if (local_addr) {
            ...
            gex_AM_RequestShort(upcri_tm, node,
	         UPCRI_HANDLER_ID(upcri_SRQ_freeheldlock), 0,
...
#endif
void upcri_SRP_lockreply(gex_Token_t token
                         UPCR_LOCK_TID_ARG(gex_AM_Arg_t threadid),
                         gex_AM_Arg_t generation,
                         gex_AM_Arg_t thread, void *addr) {
  #if UPCRI_AMLOCKS
...
#endif
void upcri_SRP_unlockreply(gex_Token_t token
                           UPCR_LOCK_TID_ARG(gex_AM_Arg_t threadid),
                           gex_AM_Arg_t reply) {
  #if UPCRI_AMLOCKS
    UPCR_LOCK_TID_OF_RCVR(threadid)
...
/* -------------------------------------------------------------------------- */
#if UPCRI_AMLOCKS
GASNETT_INLINE(upcri_doSRQ_lockgrant)
void upcri_doSRQ_lockgrant(upcri_shared_lockinfo_t * const sli) {
    upcri_assert(sli->blocked);
    sli->blocked = 0;
}
#endif
void upcri_SRQ_lockgrant(gex_Token_t token
                           UPCR_LOCK_TID_ARG(gex_AM_Arg_t threadid)) {
  #if UPCRI_AMLOCKS
    UPCR_LOCK_TID_OF_RCVR(threadid)
    upcri_shared_lockinfo_t *his_sli = upcri_his_sharedlockinfo(threadid);
    upcri_doSRQ_lockgrant(his_sli);
  #else
    upcri_err("Bad call to upcri_doSRQ_lockgrant");
  #endif
}
/* -------------------------------------------------------------------------- */
#if UPCRI_AMLOCKS
...
#endif
void upcri_SRQ_lockenqueue(gex_Token_t token,
                           gex_AM_Arg_t next_thread,
                           void *_addr) {
  #if UPCRI_AMLOCKS
...
void upcri_doSRQ_lock(gex_AM_Arg_t isblocking, upcri_lock_t *lock,
                      gex_AM_Arg_t thread, upcri_heldlock_t *address,
...
void upcri_SRQ_lock(gex_Token_t token,
                    gex_AM_Arg_t isblocking, void *_lock,
                    gex_AM_Arg_t threadid, void *_address) {
#if !UPCRI_AMLOCKS
    ...
    if (his_sli != NULL) {
        upcri_assert(upcri_thread_to_node(threadid) != upcri_mynode);
...
int
upcri_doSRQ_unlock(gex_AM_Arg_t threadid,
                   gex_AM_Arg_t generation,
                   upcri_lock_t *lock) {
...
void upcri_SRQ_unlock(gex_Token_t token,
                      gex_AM_Arg_t threadid,
                      gex_AM_Arg_t generation,
		      void *_lock) {
#if !UPCRI_AMLOCKS
    ...
    {
        gex_AM_ReplyShort(token,
                       UPCRI_HANDLER_ID(upcri_SRP_unlockreply), 0
...
int _upcri_lock(upcr_shared_ptr_t lockptr, int isblocking UPCRI_PT_ARG) {
    gex_Rank_t node = upcri_shared_nodeof(lockptr);
    upcri_lock_t *lockaddr = upcri_shared_to_remote(lockptr);
    upcri_lockinfo_t * const li = &(upcri_auxdata()->lock_info);
  #if UPCRI_AMLOCKS
    upcri_shared_lockinfo_t * const sli = li->shared;
    ...
    if (localaddr) {
      ...
      gex_AM_RequestShort(upcri_tm, node,
		       UPCRI_HANDLER_ID(upcri_SRQ_lock), 0,
    ...
    if (isblocking && !success) { /* enqueue self and then wait */
        ...
        if (remote_addr != NULL) {
            ...
            if (local_addr) {
                ...
                gex_AM_RequestShort(upcri_tm, node,
		               UPCRI_HANDLER_ID(upcri_SRQ_lockenqueue), 0,
  ...
  if (isblocking || success) {
    ...
    #endif
    #if GASNET_STATS
      if (node == upcri_mynode) (li->locklocal_cnt)++;
    ...
    #endif
    #if GASNET_STATS
      if (node == upcri_mynode) (li->lockfaillocal_cnt)++;
...
void _upcr_unlock(upcr_shared_ptr_t lockptr UPCRI_PT_ARG) {
  ...
  #ifdef GASNET_TRACE
  {
  ...
  #if GASNET_STATS
    if (node == upcri_mynode) (li->unlocklocal_cnt)++;
    ...
    if (held_lock->next_thread != UPCRI_INVALID_THREAD) {
        ...
        if (localaddr) {
            ...
            gex_AM_RequestShort(upcri_tm, node,
                             UPCRI_HANDLER_ID(upcri_SRQ_unlock), 0,
    ...
    if (!isfree) {
        ...
        if (his_sli != NULL) {
          ...
          gex_AM_RequestShort(upcri_tm, node,
		        UPCRI_HANDLER_ID(upcri_SRQ_lockgrant), 0
